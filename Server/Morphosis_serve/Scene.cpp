...
{
	for (int i = 0; i < m_nProjectileObjects; i++) if (!m_ppProjectileObjects[i]->IsDead()) m_ppProjectileObjects[i]->MoveOOBB(fTimeElapsed);
...
void CPlayScene::Initialize(ID3D12Device * pd3dDevice, ID3D12GraphicsCommandList * pd3dCommandList, void * pContext)
{
	...
	//	Network
	if (0 != m_pFramework->m_NetModule.Connect()){
		cout << "Network Connect Error\n";
	}
	else{
		cout << "Network Connect Success\n";
	}
...
//	SetCursorPos(m_ptOldCursorPos.x, m_ptOldCursorPos.y);
	...
	if (pKeysBuffer[KEY::W] & 0xF0) { xmf3temp = m_ppPlayers[0]->GetLook(); (Vector3::ScalarProduct(xmf3temp, MOVE_SPEED)); }
	if (pKeysBuffer[KEY::A] & 0xF0) { xmf3temp = m_ppPlayers[0]->GetRight();(Vector3::ScalarProduct(xmf3temp, -MOVE_SPEED)); }
	if (pKeysBuffer[KEY::S] & 0xF0) { xmf3temp = m_ppPlayers[0]->GetLook(); (Vector3::ScalarProduct(xmf3temp, -MOVE_SPEED)); }
	if (pKeysBuffer[KEY::D] & 0xF0) { xmf3temp = m_ppPlayers[0]->GetRight();(Vector3::ScalarProduct(xmf3temp, MOVE_SPEED)); }
	
	if (xmf3temp.x != 0 || xmf3temp.y != 0 || xmf3temp.z != 0)
	{
		NetModule* nm = &m_pFramework->m_NetModule;
		CS_Move_Packet *mp = reinterpret_cast<CS_Move_Packet*>(nm->getSendBuffer());
		mp->type = CS_MOVE;
		mp->size = sizeof(CS_Move_Packet);
		mp->x = xmf3temp.x;
		mp->y = xmf3temp.y;
		mp->z = xmf3temp.z;
		nm->SendPacket(mp, sizeof(CS_Move_Packet));
		xmf3temp.x = 0;
		xmf3temp.y = 0;
		xmf3temp.z = 0;
	}
...
void CPlayScene::GetPacket(void * packet)
{
	if (packet != NULL)
	{
		...
		{
			//cout << "move\n";
			SC_Move_Packet *mp = reinterpret_cast<SC_Move_Packet*>(packet);
			XMFLOAT3 temp;
			temp.x = mp->x;
			temp.y = mp->y;
			temp.z = mp->z;
			if (mp->key == m_pFramework->m_NetModule.key)
			{
				m_ppPlayers[0]->AddPosVariation(temp);
			}
			else
			{
				m_ppPlayers[1]->AddPosVariation(temp);
			}
