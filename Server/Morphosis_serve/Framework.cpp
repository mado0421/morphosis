...
LRESULT CFramework::OnProcessingWindowMessage(HWND hWnd, UINT nMessageID, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_KEYUP:
		...
		break;
	case WM_SOCKET:
		m_NetModule.Process_WM_SOCKET(wParam, lParam);
		if (m_NetModule.needFlag)
		{
			m_pCurrentScene->GetPacket(m_NetModule.getRecvBuffer());
			m_NetModule.needFlag = false;
		}
		break;
	}
	return(0);
