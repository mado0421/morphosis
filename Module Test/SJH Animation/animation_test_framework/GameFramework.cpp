...
void CGameFramework::BuildObjects()
{
	...
	FBXDataManager *pFbxManager = new FBXDataManager;
	pFbxManager->MeshFileRead("FreeVoxelGirlBlackhair_test");
	pFbxManager->SetTextureName("FreeVoxelGirlBlackhair2");
	m_pScene->m_pPlayer = m_pPlayer = new CAirplanePlayer(m_pd3dDevice, m_pd3dCommandList, m_pScene->GetGraphicsRootSignature(), pFbxManager);
	//m_pPlayer->Rotate(270, 0, 0);
...
void CGameFramework::ProcessInput()
{
	...
	if (!bProcessedByScene)
	{
		...
		if (GetCapture() == m_hWnd)
		{
			...
			cxDelta = (float)(ptCursorPos.x - m_ptOldCursorPos.x) / 9.0f;
			cyDelta = (float)(ptCursorPos.y - m_ptOldCursorPos.y) / 9.0f;
...
void CGameFramework::AnimateObjects()
{
	...
	if (m_pScene) m_pScene->AnimateObjects(fTimeElapsed,m_pd3dCommandList);
}
