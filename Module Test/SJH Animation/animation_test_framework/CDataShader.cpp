...
D3D12_INPUT_LAYOUT_DESC CDataShader::CreateInputLayout()
{
...
D3D12_SHADER_BYTECODE CDataShader::CreateVertexShader(ID3DBlob ** ppd3dShaderBlob)
{
	return(CShader::CompileShaderFromFile(L"Shaders.hlsl", "VSData", "vs_5_1", ppd3dShaderBlob));
}

D3D12_SHADER_BYTECODE CDataShader::CreatePixelShader(ID3DBlob ** ppd3dShaderBlob)
{
	return(CShader::CompileShaderFromFile(L"Shaders.hlsl", "PSData", "ps_5_1", ppd3dShaderBlob));
}

void CDataShader::CreateShader(ID3D12Device * pd3dDevice, ID3D12RootSignature * pd3dGraphicsRootSignature)
{
	m_nPipelineStates = 1;
