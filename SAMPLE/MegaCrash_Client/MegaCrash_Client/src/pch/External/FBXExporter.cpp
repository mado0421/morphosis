...
void FBXExporter::ExportFBX()
{
	...
	mGenericFileName = FBXUtilities::GetFileName(mInputFilePath);
	mGenericFileName = FBXUtilities::RemoveSuffix(mGenericFileName);

	mProcessGeometryCount = 0;

	std::cout << "\n\n\n\nExporting Model:" << mGenericFileName << "\n";
	...
	std::cout << "ProcessGeometryCount: " << mProcessGeometryCount << std::endl;
...
void FBXExporter::ProcessSkeletonHierarchyRecursively(FbxNode* inNode, int inDepth, int myIndex, int inParentIndex)
{
	...
	{
		if (inNode->GetNodeAttribute()->GetAttributeType() == FbxNodeAttribute::eSkeleton)
		{
			FBXJoint currJoint;
			currJoint.mParentIndex = inParentIndex;
			currJoint.mName = inNode->GetName();
			mOutputDataContainer[mProcessGeometryCount].mSkeleton.mJoints.push_back(currJoint);
		}
	}
	for (int i = 0; i < inNode->GetChildCount(); i++)
	{
		ProcessSkeletonHierarchyRecursively(inNode->GetChild(i), inDepth + 1, mOutputDataContainer[mProcessGeometryCount].mSkeleton.mJoints.size(), myIndex);
	}
}
...
void FBXExporter::ProcessJointsAndAnimations(FbxNode* inNode)
{
	...
	FbxAMatrix geometryTransform = FBXUtilities::GetGeometryTransformation(inNode);
	...
	for (unsigned int deformerIndex = 0; deformerIndex < numOfDeformers; ++deformerIndex)
	{
		...
		for (unsigned int clusterIndex = 0; clusterIndex < numOfClusters; ++clusterIndex)
		{
			...
			mOutputDataContainer[mProcessGeometryCount].mSkeleton.mJoints[currJointIndex].mGlobalBindposeInverse = globalBindposeInverseMatrix;
			mOutputDataContainer[mProcessGeometryCount].mSkeleton.mJoints[currJointIndex].mNode = currCluster->GetLink();
			...
			mOutputDataContainer[mProcessGeometryCount].mAnimationName = animStackName.Buffer();
			FbxTakeInfo* takeInfo = mFBXScene->GetTakeInfo(animStackName);
			FbxTime start = takeInfo->mLocalTimeSpan.GetStart();
			FbxTime end = takeInfo->mLocalTimeSpan.GetStop();
			mOutputDataContainer[mProcessGeometryCount].mAnimationLength = end.GetFrameCount(FbxTime::eFrames24) - start.GetFrameCount(FbxTime::eFrames24) + 1;
			FBXKeyframe** currAnim = &mOutputDataContainer[mProcessGeometryCount].mSkeleton.mJoints[currJointIndex].mAnimation;
...
int FBXExporter::FindJointIndexUsingName(const std::string& inJointName)
{
	for(unsigned int i = 0; i < mOutputDataContainer[mProcessGeometryCount].mSkeleton.mJoints.size(); ++i)
	{
		if (mOutputDataContainer[mProcessGeometryCount].mSkeleton.mJoints[i].mName == inJointName)
		{
			return i;
		}
	}
...
void FBXExporter::ProcessMesh(FbxNode* inNode)
{
	...
	mOutputDataContainer[mProcessGeometryCount].mTriangleCount = currMesh->GetPolygonCount();
	int vertexCounter = 0;
	mOutputDataContainer[mProcessGeometryCount].mTriangles.reserve(mOutputDataContainer[mProcessGeometryCount].mTriangleCount);

	for (unsigned int i = 0; i < mOutputDataContainer[mProcessGeometryCount].mTriangleCount; ++i)
	{
		XMFLOAT3 normal[3];
		XMFLOAT3 tangent[3];
		XMFLOAT3 binormal[3];
		...
		mOutputDataContainer[mProcessGeometryCount].mTriangles.push_back(currTriangle);
		...
		for (unsigned int j = 0; j < 3; ++j)
		{
			...
			{
				FBXVertexBlendingInfo currBlendingInfo;
			...
			mOutputDataContainer[mProcessGeometryCount].mVertices.push_back(temp);
			mOutputDataContainer[mProcessGeometryCount].mTriangles.back().mIndices.push_back(vertexCounter);
			++vertexCounter;
		}
	}
...
{
	for (auto& p : mOutputDataContainer)
	{
		// First get a list of unique vertices
		std::vector<FBXPNTIWVertex> uniqueVertices;
		for (unsigned int i = 0; i < p.mTriangles.size(); ++i)
		{
			for (unsigned int j = 0; j < 3; ++j)
			{
				...
				p.mVertices[idx].mMaterialID = static_cast<unsigned char>(p.mTriangles[i].mMaterialIndex);
				if (FindVertex(p.mVertices[idx], uniqueVertices) == -1)
				{
					uniqueVertices.push_back(p.mVertices[idx]);
				}
			}
		}
		...
		for (unsigned int i = 0; i < p.mTriangles.size(); ++i)
		{
			for (unsigned int j = 0; j < 3; ++j)
			{
				p.mTriangles[i].mIndices[j] = FindVertex(p.mVertices[i * 3 + j], uniqueVertices);
			}
		}

		p.mVertices.clear();
		p.mVertices = uniqueVertices;
		uniqueVertices.clear();

		// Now we sort the triangles by materials to reduce 
		// shader's workload
		std::sort(p.mTriangles.begin(), p.mTriangles.end());
	}
}

int FBXExporter::FindVertex(const FBXPNTIWVertex& inTargetVertex, const std::vector<FBXPNTIWVertex>& uniqueVertices)
{
...
{
	CleanupFbxManager();
	std::vector<Vertex::FBXPNTVertex> newVertices;
...
int FBXExporter::FindVertex(const Vertex::FBXPNTVertex& inTarget, const std::vector<Vertex::FBXPNTVertex>& inVertices)
{
	int index = -1;
...
void FBXExporter::AssociateMaterialToMesh(FbxNode* inNode)
{
	...
	if(currMesh->GetElementMaterial())
	{
		...
		if(materialIndices)
		{
			...
			{
				if (materialIndices->GetCount() == mOutputDataContainer[mProcessGeometryCount].mTriangleCount)
				{
					for (unsigned int i = 0; i < mOutputDataContainer[mProcessGeometryCount].mTriangleCount; ++i)
					{
						unsigned int materialIndex = materialIndices->GetAt(i);
						mOutputDataContainer[mProcessGeometryCount].mTriangles[i].mMaterialIndex = materialIndex;
					}
				}
			}
			break;
...
void FBXExporter::ProcessMaterials(FbxNode* inNode)
{
	...
	for(unsigned int i = 0; i < materialCount; ++i)
	{
		...
		ProcessMaterialTexture(surfaceMaterial, mOutputDataContainer[mProcessGeometryCount].mMaterialLookUp[i]);
	}
}
...
void FBXExporter::ProcessMaterialAttribute(FbxSurfaceMaterial* inMaterial, unsigned int inMaterialIndex)
{
	...
	{
		FBXPhongMaterial* currMaterial = new FBXPhongMaterial();
		...
		mOutputDataContainer[mProcessGeometryCount].mMaterialLookUp[inMaterialIndex] = currMaterial;
	}
	else if (inMaterial->GetClassId().Is(FbxSurfaceLambert::ClassId))
	{
		FBXLambertMaterial* currMaterial = new FBXLambertMaterial();
...
{
	for (auto& p : mOutputDataContainer)
		for(auto itr = p.mMaterialLookUp.begin(); itr != p.mMaterialLookUp.end(); ++itr)
		{
...
void FBXExporter::PrintTriangles()
{
	for (auto& p : mOutputDataContainer)
		for(unsigned int i = 0; i < p.mTriangles.size(); ++i)
		{
			std::cout << "Triangle# " << i + 1 << " FBXMaterial Index: " << p.mTriangles[i].mMaterialIndex << "\n";
		}
}
...
void FBXExporter::CleanupFbxManager()
{
	...
	for (auto& p : mOutputDataContainer)
	{
		p.mTriangles.clear();
		p.mVertices.clear();
		p.mSkeleton.mJoints.clear();

		for (auto itr = p.mMaterialLookUp.begin(); itr != p.mMaterialLookUp.end(); ++itr)
		{
			delete itr->second;
		}
		p.mMaterialLookUp.clear();
	}
	mOutputDataContainer.clear();
}
/*
...
void FBXExporter::WriteAnimationToStream(std::ostream& inStream)
{
	...
	for (unsigned int i = 0; i < mSkeleton.mJoints.size(); ++i)
	{
		...
		FBXUtilities::WriteMatrix(inStream, out.Transpose(), true);
		inStream << "\t\t</joint>\n";
	}
	inStream << "\t</skeleton>\n";
	inStream << "\t<animations>\n";
