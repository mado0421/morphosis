...
void FBXUtilities::WriteMatrix(std::ostream& inStream, FbxMatrix& inMatrix, bool inIsRoot)
{
...
void FBXUtilities::PrintMatrix(FbxMatrix& inMatrix)
{
...
FbxAMatrix FBXUtilities::GetGeometryTransformation(FbxNode* inNode)
{
	if (!inNode)
	{
...
std::string FBXUtilities::GetFileName(const std::string& inInput)
{
...
std::string FBXUtilities::RemoveSuffix(const std::string& inInput)
{
	std::string seperator(".");
	unsigned int pos = inInput.find_last_of(seperator);
	if (pos != std::string::npos)
