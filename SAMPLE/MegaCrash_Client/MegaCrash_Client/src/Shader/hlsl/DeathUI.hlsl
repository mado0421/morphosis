...
cbuffer cbDeathUIInfo : register(b4)
{
    float4 UI_Color;
    float StartTime;
};
Texture2D gtxtDeathTexture[10] : register(t2);
...
float4 PS_MAIN(VS_OUTPUT input) : SV_Target
{
    ...
    float4 result_color = gtxtDeathTexture[0].Sample(gLinearWrapSampler, input.uv);
    ...
    result_color.r += saturate(length(input.uv) * sin(gfTimer)*2);
