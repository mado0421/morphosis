...
Texture2D gtxtCommonTexture[10] : register(t2);
Texture2D gtxtHighLightTexture[10] : register(t12);
...
{
    if (bCursorOn == 1)
        return gtxtHighLightTexture[0].Sample(gLinearWrapSampler, input.uv) * (1.125 + sin(gfTimer*3.5f) * 0.125);
    else
        return gtxtCommonTexture[0].Sample(gLinearWrapSampler, input.uv);
}