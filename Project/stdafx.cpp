...
void CreateModel(const char * meshName, const char * textureName, const char* modelName)
{
	...
	model->m_MeshIdx	= -1;
	model->m_TextureIdx = -1;

	for (int i = 0; i < g_vecMesh.size(); ++i) if (g_vecMesh[i]->m_MeshName == meshName)				model->m_MeshIdx = i;
	for (int i = 0; i < g_vecTexture.size(); ++i) if (g_vecTexture[i]->m_textureName == textureName)	model->m_TextureIdx = i;
