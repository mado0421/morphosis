...
// ±âº»ÀûÀÎ ¿ÀºêÁ§Æ®µé Àü¿ë(Å©±â º¯µ¿ÀÌ ¾ø´Â ±âº»ÀûÀÎ UI
...
public:
	void ExportFile(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList, CTexture* pTexture, const char* fileName, CAnimationPlayerObject& obj) {
		...
		/***************************************************************
		bone Á¤º¸ ¾²°í mesh Á¤º¸ ¾²°í key Á¤º¸ ¾²°í
		***************************************************************/
		/***************************************************************
		°Á ¸î °³ ÀÖ´ÂÁö¶û nameÀÌ¶û ºÎ¸ðboneIdx¸¸ ÀúÀåÇÏÀÚ.
		ºÎ¸ð ¾øÀ¸¸é ºÎ¸ð boneIdx °ª -1ÀÓ
		***************************************************************/
		int nBones;
		...
		/***************************************************************
		meshº°·Î ControlPoint ´Ù ½á¾ßµÊ.
		float3, int4, float4
		±×°ÅÇÏ°í polygonVertexIndexµµ ¾²±â
		***************************************************************/
		int nMeshes;
		...
		/***************************************************************
		Å°°¡ ¸î °³ ÀÖ°í, °¢ Å°¸¶´Ù BoneÀÌ ¸î °³ ÀÖÀ» °ÍÀÎÁö ÀúÀåÇØ¾ß ÇÔ.
		ÀÏ´ÜÀº ¸ðµç BoneÀÌ xyz ¿ä¼Ò°ªÀ» °¡Áö´Â °ÍÀ¸·Î °£ÁÖÇÏ°í ÀÛ¼ºÇÏÀÚ.
		***************************************************************/
		int nKeys;
		...
		for (int i = 0; i < nKeys; ++i) {
			...
			keys[i].m_pBones.resize(nBones); // ¾µ ÀÏÀº ¾øÁö¸¸ ÈÄ¿¡ resize() ÇÔ¼ö È£Ãâ ½Ã¿¡ Á÷°ü¼ºÀ» À§ÇØ
...
=======
				swap(keys[i].m_translations[j].y, keys[i].m_translations[j].z);
				swap(keys[i].m_rotations[j].y, keys[i].m_rotations[j].z);
>>>>>>> parent of e36c215... Merge branch 'ìƒˆë§ˆì„ìš´ë™'
			}
		}
...
public:
	...
	virtual void Initialize(XMFLOAT3 pos, XMFLOAT3 dir, UINT64 nCbvGPUDescriptorHandlePtr, XMFLOAT3 extents, void * pAddress = nullptr) {
		...
		// ¿©±â¼­ m_pCollisionModel ³Ö¾îÁà¾ß µÊ~~~~
	}
...
public:
	// ÀÌµ¿ °´Ã¼´Â ¿òÁ÷ÀÏ ¹æÇâ°ú ÀÌµ¿ ¼Óµµ, Áß·Â ¿µÇâ À¯¹«(°¡´ÉÇÏ´Ù¸é °è¼ö±îÁö)

	// ProcessInput()¿¡¼­ È¸Àü°¢°ú ÀÌµ¿·®À» Á¤ÇØÁÖ¸é Update()¿¡¼­ Èå¸¥ ½Ã°£°ú °öÇÏ¿© ½ÇÁ¦ ÀÌµ¿ÇÏ±â.
	// È¸ÀüÇÏ¸é ¹Ù²î´Â °Í: ¹æÇâ º¤ÅÍ?
...
protected:
	// ÇÊ¿äÇÑ °Íµé: ¹æÇâ º¤ÅÍ, ÇöÀç ¼öÆò ±âÁØ »óÇÏ È¸Àü°¢
	// ±Ùµ¥ ¹æÇâ º¤ÅÍ´Â look Vector ¾Æ´Ñ°¡? ÇÊ¿ä¾øÀ»°Å °°À½ ¶¥¶¥¶¥.
	...
	// Áß·Â ¿µÇâ ¹Þ³ª¿ä?? 100% ¹ÞÀ¸¸é 1.0f, ¾È ¹ÞÀ¸¸é 0.0f
...
public:
	UINT GetNumTotalObjects() {
		// levelData°¡ ÀÖ´Ù¸é µ¥ÀÌÅÍ¸¦ ÀÐ¾î¼­ Á¤È®ÇÑ Object °³¼ö¸¦ ±¸ÇÒ ¼ö ÀÖ´Ù.
		// TerrainMeshes + Props + TerrainCollisionBoxes µî
		// ±Ùµ¥ ±×°Ç Áö±Ý ¾øÀ¸´Ï±î ÀÏ´ÜÀº ÇÏ³ª·Î ÇÏÀÚ.
		return 1 + 0 + 1 +
	...
	// terrainMeshes, terrainCollisionBoxes, props, light, spawnPoint, targetArea, texture µî¿¡¼­
	// terrainMeshes, terrainCollisionBoxes, props, texture¸¦ ´ã´ç.
